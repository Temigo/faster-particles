...
from faster_particles.ppn_utils import include_gt_pixels, \
    compute_positives_ppn2, compute_positives_ppn1, \
    assign_gt_pixels, generate_anchors, \
...
class PPN(object):
    ...
    def __init__(self, cfg=None, base_net=VGG, base_net_args={}):
        ...
        # Global parameters
        self.R = cfg.R
        self.num_classes = 2  # cfg.NUM_CLASSES
    ...
    def test_image(self, sess, blob):
        feed_dict = {
            ...
            self.gt_pixels_placeholder: blob['gt_pixels']
            }
    ...
    def train_step_with_summary(self, sess, blobs):
        ...
        if np.isnan(loss):
            ...
            print("ppn1_closest_gt_distance: ", ppn1_closest_gt_distance)
    ...
    def set_dimensions(self, f3shape, f5shape):
        ...
        print("N2 = %d ; N3 = %d ; dim1 = %d ; dim2 = %d" %
              (self.N2, self.N3, self.dim1, self.dim2))
    ...
    def create_architecture(self, is_training=True, reuse=None, scope="ppn"):
        ...
        with slim.arg_scope([slim.conv2d, slim.conv3d, slim.fully_connected],
            ...
            net, net2 = self.base_net.build_base_net(
            ...
            with tf.variable_scope(scope, reuse=self.reuse):
                ...
                if self.is_training:
                    # During training time, check if all
                    # ground truth pixels are covered by ROIs
                    # If not, add relevant ROIs on F3
                    rois = include_gt_pixels(rois,
                                             self.get_gt_pixels(),
                                             self.dim1, self.dim2)
                ...
                with tf.variable_scope("final_proposals"):
                    im_proposals = tf.identity(
                        (proposals2 + self.dim2*rois)*self.dim1,
                        name="im_proposals_raw")
                    im_labels = tf.argmax(scores2,
                                          axis=1, name="im_labels_raw")
                    im_scores = tf.reshape(scores2, (-1,))
                    # We have now num_roi proposals and corresponding
                    # labels in original image.

                    # Select proposals above a minimum score.
                    keep = tf.where(im_scores > self.cfg.MIN_SCORE,
                                    name="keep_good_scores")[:, 0]
                    keep = tf.reshape(keep, (-1, 1))
                    im_proposals = tf.gather_nd(im_proposals, keep,
                                                name="im_proposals")
                    ...
                    if self.cfg.POSTPROCESSING == 'nms':  # Pixel NMS equivalent
                        ...
                        im_proposals, im_scores, keep = tf.py_func(
                            ...
                            [im_proposals,
                             im_scores,
                             15.0 if self.cfg.DATA_3D else 20.0],
                ...
                if self.is_training:
                    ...
                    closest_distance = tf.reduce_min(distances,
                                                     axis=1,
                                                     name="final_closest_distance")
                ...
                if self.is_training:
                    ...
                    tf.summary.scalar('distance_final_point', self._losses['distance_final_point'])

                    relative_nb_points = tf.shape(im_proposals)[0] / tf.shape(self.gt_pixels_placeholder)[0]
                    tf.summary.scalar('relative_nb_points', relative_nb_points)
    ...
    def build_ppn1(self, net2):
        ...
        with tf.variable_scope("ppn1", reuse=self.reuse):
            ...
            # Derive predicted positions (poi)
            # with scores (poi_scores) from prediction parameters
            # and anchors. Take the first R proposed pixels
            # which contain an object.
            proposals, scores = predicted_pixels(ppn1_cls_prob,
                                                 ppn1_pixel_pred,
                                                 anchors,
                                                 (self.N2,) * self.dim)
            ...
            # if self.is_training:
            # all outputs from 1x1 convolution are categorized
            ...
            closest_gt, closest_gt_distance, _ = assign_gt_pixels(
                self.gt_pixels_placeholder, proposals, self.dim1, self.dim2)
            ...
            # First is point loss: for positive pixels, distance from
            # proposed pixel to closest ground truth pixel
            # FIXME Use smooth L1 for distance loss?
            loss_ppn1_point = tf.reduce_mean(tf.reduce_mean(tf.boolean_mask(
                closest_gt_distance, classes_mask)), name="loss_ppn1_point")
            labels_ppn1 = tf.cast(tf.reshape(classes_mask, (-1,)), tf.int32,
                                  name="labels_ppn1")
            loss_ppn1_class = tf.reduce_mean(tf.reduce_sum(
                tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=labels_ppn1,
                    logits=tf.reshape(ppn1_cls_score, (-1, 2))
                    )), name="loss_ppn1_class")
            accuracy_ppn1 = tf.reduce_mean(tf.cast(tf.equal(
                tf.cast(tf.argmax(tf.reshape(ppn1_cls_prob, (-1, 2)), axis=1),
                        tf.int32),
                labels_ppn1), tf.float32), name="accuracy_ppn1")
    ...
    def build_ppn2(self, rpn_pooling, rois):
        ...
        with tf.variable_scope("ppn2", reuse=self.reuse):
            # Define initializers
            initializer2 = tf.truncated_normal_initializer(mean=0.0,
                                                           stddev=0.01)
            # batch size should be number of rois x number of pixels per roi
            ...
            # Derive proposed points from delta predictions
            # (ppn2_pixel_pred) w.r.t. pixels centers
            # Coordinates of proposals2 are in 1x1 ROI area
            # We have 1*1*num_roi proposals and corresponding scores
            proposals2, scores2 = predicted_pixels(ppn2_cls_prob,
                                                   ppn2_pixel_pred,
                                                   anchors2, (1,)*self.dim)
            ...
            # Option roi allows to convert gt_pixels_placeholder
            # information to ROI 4x4 coordinates
            closest_gt, closest_gt_distance, true_labels = assign_gt_pixels(
                self.gt_pixels_placeholder,
                proposals2,
                self.dim1, self.dim2, rois=rois)
            ...
            positives = compute_positives_ppn2(
                closest_gt_distance,
                threshold=self.ppn2_distance_threshold)
            ...
            loss_ppn2_point = tf.reduce_mean(tf.reduce_mean(tf.boolean_mask(
                closest_gt_distance, positives)), name="loss_ppn2_point")
            ...
            labels_ppn2 = tf.cast(tf.reshape(
                tf.cast(positives, tf.float32)*true_labels,
                (-1,)
                ), tf.int32, name="labels_ppn2")
            labels_ppn2 = tf.where(labels_ppn2 > 0, tf.ones_like(labels_ppn2),
                                   tf.zeros_like(labels_ppn2))
            ...
            if self.cfg.WEIGHT_LOSS:
                ...
                shower_indices = tf.where(tf.equal(labels_ppn2, tf.constant(2)),
                                          name="shower_indices")
                background_indices = tf.where(tf.equal(
                    labels_ppn2,
                    tf.constant(0)), name="background_indices")

                labels_ppn2_track = tf.gather(labels_ppn2, track_indices,
                                              name="labels_ppn2_track")
                labels_ppn2_shower = tf.gather(labels_ppn2, shower_indices,
                                               name="labels_ppn2_shower")
                labels_ppn2_background = tf.gather(labels_ppn2,
                                                   background_indices,
                                                   name="labels_ppn2_background")

                logits_track = tf.gather(logits, track_indices,
                                         name="logits_track")
                logits_shower = tf.gather(logits, shower_indices,
                                          name="logits_shower")
                logits_background = tf.gather(logits, background_indices,
                                              name="logits_background")

                loss_ppn2_background = tf.cond(
                    tf.equal(tf.shape(background_indices)[0], tf.constant(0)),
                    false_fn=lambda: tf.reduce_mean(tf.reduce_mean(
                        tf.nn.sparse_softmax_cross_entropy_with_logits(
                            labels=labels_ppn2_background,
                            logits=logits_background))),
                    true_fn=lambda: 0.0, name="loss_ppn2_background")
                loss_ppn2_track = tf.cond(
                    tf.equal(tf.shape(track_indices)[0], tf.constant(0)),
                    false_fn=lambda: tf.reduce_mean(tf.reduce_mean(
                        tf.nn.sparse_softmax_cross_entropy_with_logits(
                            labels=labels_ppn2_track,
                            logits=logits_track))),
                    true_fn=lambda: 0.0, name="loss_ppn2_track")
                loss_ppn2_shower = tf.cond(
                    tf.equal(tf.shape(shower_indices)[0], tf.constant(0)),
                    false_fn=lambda: tf.reduce_mean(tf.reduce_mean(
                        tf.nn.sparse_softmax_cross_entropy_with_logits(
                            labels=labels_ppn2_shower,
                            logits=logits_shower))),
                    true_fn=lambda: 0.0, name="loss_ppn2_shower")

                gt_labels = tf.slice(self.gt_pixels_placeholder,
                                     [0, self.dim], [-1, 1], name="gt_labels")
                nb_tracks = tf.reduce_sum(tf.cast(
                    tf.equal(gt_labels, 1), tf.float32), name="nb_tracks")
                nb_showers = tf.reduce_sum(tf.cast(
                    tf.equal(gt_labels, 2), tf.float32), name="nb_showers")
                loss_ppn2_class = tf.identity(
                    loss_ppn2_background
                    + nb_tracks / (nb_tracks + nb_showers) * loss_ppn2_track
                    + nb_showers / (nb_tracks + nb_showers) * loss_ppn2_shower,
                    name="loss_ppn2_class")
            else:
                loss_ppn2_class = tf.reduce_mean(tf.reduce_sum(
                    tf.nn.sparse_softmax_cross_entropy_with_logits(
                        labels=labels_ppn2,
                        logits=logits)), name="loss_ppn2_class")

            accuracy_ppn2 = tf.reduce_mean(tf.cast(tf.equal(
                tf.cast(tf.argmax(
                    tf.reshape(ppn2_cls_prob, (-1, self.num_classes)),
                    axis=1), tf.int32),
                labels_ppn2), tf.float32))
    ...
    def get_gt_pixels(self):
        ...
        return tf.slice(self.gt_pixels_placeholder, [0, 0], [-1, self.dim],
                        name="gt_pixels_coord")
